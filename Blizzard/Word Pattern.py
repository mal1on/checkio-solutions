def check_command(pattern, command):
    ...
    formated = str(bin(pattern))[2:].zfill(len(command))

    for ind, char in enumerate(command):
        if char.isdigit() and formated[ind] == '1':
            return False
        elif char.isalpha() and formated[ind] == '0':
            return False
    return True if len(formated) == len(command) else False
